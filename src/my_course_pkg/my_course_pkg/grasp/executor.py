...
class MoveItMotionError(RuntimeError):
    def __init__(self, step_name):
        ...
        super().__init__(
            f"MoveIt could not reach the target for step {step_name!r}."
        )
...
class VerticalApproachConvergenceError(RuntimeError):
    def __init__(
        ...
        if z_decision is None:
            ...
            self.z_negative_tolerance_m = float(
                z_decision.negative_tolerance_m
            )
            self.z_positive_tolerance_m = float(
                z_decision.positive_tolerance_m
            )
...
class ArmMotionExecutor:
    ...
    def get_current_ee_pose_6d(self):
    ...
    def move_linear(self, start_pose_6d, end_pose_6d):
        ...
        if step.action == "move":
            if step.mode == "moveit":
                ...
                if not self.move_with_moveit(step.end_pose_6d):
                    message = (
                        f"MoveIt failed during step {step.name!r}; "
                        "stopping before any Cartesian fallback."
                    )
                    self.node.get_logger().error(message)
                    raise MoveItMotionError(step.name)
            elif step.mode == "cartesian":
                self.enter_servo_pos_mode()
                self.move_linear(step.start_pose_6d, step.end_pose_6d)
            else:
    ...
    def _gripper_result_is_open(cls, result, target_position):
        ...
        actual_position = cls._gripper_result_field(
            result,
            "actual_position",
            None,
        )
    ...
    def _validate_gripper_result(self, step, result):
        if not self._gripper_result_accepted(result):
            self._raise_gripper_error(
                step,
                result,
                "command result was not accepted",
            )
    ...
    def _validate_pear_close_result(self, plan, step, result):
        ...
        actual_position = self._gripper_result_field(
            result,
            "actual_position",
            None,
        )
        ...
        if actual_position + 1e-9 < minimum_position:
            self._raise_gripper_error(
                ...
                f"(actual={actual_position:.3f}, "
                f"minimum={minimum_position:.3f}, "
    ...
    def _publish_grasp_debug_markers(self, plan):
        ...
        if not hasattr(self, "grasp_debug_marker_publisher"):
            return
        ...
        if norm == 0.0:
            return
        approach_world /= norm
        geometry_center_object = np.asarray(
            debug_info.get("side_grasp_geometry_center_object", [0.0, 0.0, 0.0]),
            dtype=float,
        )
        geometry_center_point = np.array(
            [*geometry_center_object, 1.0],
            dtype=float,
        )
        geometry_center_world = (T_world_obj @ geometry_center_point)[:3]
        ...
        markers.markers.extend(
            [
                ...
                sphere(4, geometry_center_world, (0.0, 0.9, 0.9)),
            ]
        )
        arrow = Marker()
        ...
        self.grasp_debug_marker_publisher.publish(markers)
    ...
    def _log_grasp_debug(self, plan, stage):
        ...
        final_tcp = np.asarray(plan.grasp_pose_6d, dtype=float)
        pregrasp_tcp = np.asarray(plan.pre_grasp_pose_6d, dtype=float)
        targets = self._gripper_targets(plan)
        ...
        if T_world_obj is not None:
            if T_world_obj_raw is not None:
                raw_object_pose = _transform_matrix_to_pose6d(T_world_obj_raw)
                print(
                    "[GraspDebug] raw_object_pose_world_base "
                    f"{pose_text(raw_object_pose)}"
                )
                print(
                    "[GraspDebug] raw_object_local_z_axis_world="
                    f"{self._array_text(debug_info.get('raw_object_z_axis_world', []))}"
                )
            object_pose = _transform_matrix_to_pose6d(T_world_obj)
            ...
            print(
                ...
                f"{pose_text(object_pose)}"
            )
            print(
                "[GraspDebug] canonical_object_local_z_axis_world="
                f"{self._array_text(debug_info.get('canonical_object_z_axis_world', []))}"
            )
            print(
                "[GraspDebug] final_tcp_relative_to_object "
                f"dx_dy_dz={self._array_text(final_relative[:3, 3])}"
            )
            geometry_center_object = np.asarray(
                debug_info.get(
                    "side_grasp_geometry_center_object",
                    [0.0, 0.0, 0.0],
                ),
                dtype=float,
            )
            ...
            geometry_center_world = (T_world_obj @ geometry_center_point)[:3]
            final_relative_to_center = (
                final_relative[:3, 3] - geometry_center_object
            )
            print(
                "[GraspDebug] side_grasp_geometry_center_object "
                f"xyz={self._array_text(geometry_center_object)}"
            )
            print(
                "[GraspDebug] side_grasp_geometry_center_world_base "
                f"xyz={self._array_text(geometry_center_world)}"
            )
            print(
                "[GraspDebug] final_tcp_relative_to_geometry_center "
                "dx_dy_dz="
                f"{self._array_text(final_relative_to_center)}"
            )
        else:
        ...
        print(f"[GraspDebug] final_tcp_target_world_base {pose_text(final_tcp)}")
        print(f"[GraspDebug] pregrasp_tcp_target_world_base {pose_text(pregrasp_tcp)}")
        print(
            "[GraspDebug] pregrasp_distance_m="
            f"{np.linalg.norm(final_tcp[:3] - pregrasp_tcp[:3]):.4f}; "
            f"configured_approach_offset_m={APPROACH_DIST:.4f}"
        )
        print(
            "[GraspDebug] tool_plus_z_approach_axis_world="
            f"{self._array_text(debug_info.get('approach_direction_world', []))}; "
            "object="
            f"{self._array_text(debug_info.get('approach_direction_object', []))}"
        )
        print(
        ...
        if stage == "after_final_grasp":
            try:
                actual_tcp = self.get_current_ee_pose_6d()
                print(
                    "[GraspDebug] final_tcp_actual_world_base "
                    f"{pose_text(actual_tcp)}"
                )
                if T_world_obj is not None:
                    actual_relative_xyz = (
                    ...
                    print(
                        ...
                        f"dx_dy_dz={self._array_text(actual_relative_xyz)}"
                    )
                    geometry_center_object = np.asarray(
                        debug_info.get(
                            "side_grasp_geometry_center_object",
                    ...
                    actual_relative_to_center = (
                        actual_relative_xyz - geometry_center_object
                    )
                    print(
                        "[GraspDebug] final_tcp_actual_relative_to_geometry_center "
                        "dx_dy_dz="
                        f"{self._array_text(actual_relative_to_center)}"
                    )
    ...
    def execute_plan(self, plan):
        ...
        for step in plan.steps:
            ...
            if step.name == "move_to_pre_grasp" and profile == "vertical":
                ...
                vertical_command_offset_xyz = (
                    calibration.command_offset_xyz.copy()
                )
        ...
        for candidate_index, plan in enumerate(plans, start=1):
            ...
            try:
                ...
                if exc.step_name != "move_to_pre_grasp":
                    raise
                self.node.get_logger().warn(
                    f"Grasp candidate {candidate_index}/{len(plans)} is "
                    "not reachable; trying the next candidate."
                )

        raise RuntimeError(
            "No grasp candidate could reach its pre-grasp pose without "
            "a successful MoveIt plan."
        )
    ...
    def switch_to_joint_control(self):
        ...
        state_ready = (
            ...
            if services_ready
            else self.arm_api2_client.change_state_to("JOINT_TRAJ_CTL")
        )
        if not state_ready:
            self.node.get_logger().warn(
                "change_state_to_joint_ctl failed; retrying direct JOINT_TRAJ_CTL state request."
            )
            state_ready = self.arm_api2_client.change_state_to("JOINT_TRAJ_CTL")
        if not state_ready:
            self.node.get_logger().warn(
    ...
    def send_initial_pose_trajectory(self, goal_joint_state):
        ...
        if result.status == GoalStatus.STATUS_SUCCEEDED:
            self.node.get_logger().info("Direct return-to-initial trajectory completed.")
            return True
    ...
    def return_to_initial_pose(self):
        deadline = time.monotonic() + 5.0
        while not self.latest_arm_joint_names and time.monotonic() < deadline:
            time.sleep(0.1)
        ...
        goal_joint_state = VerifyInitPoseNode.build_initial_joint_goal(
            self.latest_arm_joint_names,
        ...
        for attempt in range(1, 3):
            ...
            try:
                if self.arm_api2_client.move_to_joint(goal_joint_state):
                    return True
            except Exception as exc:
                self.node.get_logger().warn(
        ...
        if self.send_initial_pose_trajectory(goal_joint_state):
            return True
