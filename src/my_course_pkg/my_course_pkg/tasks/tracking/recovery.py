...
class TargetRecoveryMixin:
    ...
    def _select_reanchored_mask(self, response_path, predicted_mask):
        ...
        interior_candidates = [
            ...
            if _mask_is_usable_away_from_image_border(mask, border_margin)
        ]
        if not interior_candidates:
            if _mask_is_usable_away_from_image_border(
                predicted_mask,
                border_margin,
            ):
                self._status(
        ...
        selected, reason = select_recovery_mask(
            interior_candidates,
            predicted_mask,
            predicted_iou=float(
    ...
    def _recover_lost_target(self):
        ...
        try:
            recovered, reason = select_recovery_mask(
                candidates,
                predicted_mask,
                predicted_iou=float(
                    self.get_parameter("reanchor_min_iou").value
