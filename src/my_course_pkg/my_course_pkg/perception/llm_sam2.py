...
SAM2_URL = os.environ.get(
    "SAM2_URL",
    "http://172.22.222.226:5000/predict",
)
        ...
        try:
            ...
            if exc.status_code not in RETRYABLE_STATUS_CODES or attempt == attempts:
                raise RuntimeError(
                    ...
                    f"HTTP {exc.status_code}: {short_response_text(exc.response)}"
                ) from exc
...
def _forced_target_object(user_instruction: str) -> str | None:
    ...
    if override:
        ...
        return target
    explicit = _explicit_object_from_instruction(user_instruction)
    if explicit is not None:
        return explicit
    alias = resolve_target_alias(user_instruction)
    return None if alias is None else alias.selected_object_name
...
"""
    ...
    has_override = bool(os.environ.get("VLM_CANDIDATE_OVERRIDE", "").strip())
    alias = None if has_override else resolve_target_alias(user_instruction)
    grounding_prompt = (
        alias.grounding_prompt
        if alias is not None
        and alias.selected_object_name == selected_object_name
        else selected_object_name
    )
    return {
        ...
        "accepted_sam2_class_names": (
            list(alias.accepted_sam2_class_names)
            if grounding_prompt != selected_object_name
            else []
        ),
        "matched_instruction_alias": (
            None if alias is None else alias.matched_instruction_alias
        ),
        "visual_target_description": (
            None if alias is None else alias.visual_target_description
        ),
    }
...
def run_sam2_api(
    ...
    with image_path.open("rb") as image_file:
        response = post_with_retries(
            SAM2_URL,
            attempts=3,
            response_text=short_response_text,
...
class LlmSam2Node:
    ...
    def run(self, user_instruction: str) -> dict:
        ...
        text_prompt = build_sam2_text_prompt([selection["grounding_prompt"]])
            ...
            json.dumps(
                {
                    ...
                    **(
                        {}
                        if selection["matched_instruction_alias"] is None
                        else {
