...
def _world_height_image(depth_m, intrinsic, T_world_cv_camera):
    ...
    world_z = camera_points @ transform[2, :3] + transform[2, 3]
    height_image = np.full(depth_m.shape, np.nan, dtype=float)
    height_image[valid] = world_z
    return height_image
...
def _random_color_bin_mask(image_bgr, min_pixels):
