...
def bounded_pbvs_target(
    ...
    *,
    ...
    """Compute one bounded proportional PBVS translation command.

    Kept as a public compatibility helper; new camera-relative code normally
    calls :func:`bounded_camera_pbvs_target`.
    """
    current_xyz = np.asarray(current_xyz, dtype=float).reshape(3)
    desired_xyz = (
        np.asarray(target_xyz, dtype=float).reshape(3)
        + np.asarray(standoff_xyz, dtype=float).reshape(3)
    )
    error = desired_xyz - current_xyz
    step, should_command = _bounded_proportional_step(
        error, gain, deadband_m, max_step_m
    )
    if not should_command:
        return current_xyz.copy(), error, False
    command = current_xyz + step
    _check_workspace(
        command,
        T_workspace_command=np.eye(4),
        workspace_min=workspace_min,
        workspace_max=workspace_max,
        command_frame="control",
        workspace_frame="control",
    )
