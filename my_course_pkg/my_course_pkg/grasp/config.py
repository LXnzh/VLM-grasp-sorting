...
def read_hammer_max_top_down_angle_deg():
    angle_deg = env_float("GRASP_HAMMER_MAX_TOP_DOWN_ANGLE_DEG", "10.0")
...
def read_final_approach_position_tolerance_m():
    ...
    if not np.isfinite(tolerance_m) or tolerance_m < 0.0:
        raise ValueError(
            "GRASP_FINAL_APPROACH_POSITION_TOLERANCE_M must be finite and "
            f"non-negative; got {tolerance_m!r}."
        )
    return tolerance_m
...
def read_vertical_reanchor_max_iterations():
    return _read_positive_integer("GRASP_VERTICAL_REANCHOR_MAX_ITERATIONS", "3")


def read_vertical_reanchor_settle_sec():
    return _read_positive_finite_float("GRASP_VERTICAL_REANCHOR_SETTLE_SEC", "0.4")
...
# Fixed fallback target in MoveIt's ``world`` frame.  The XY position is in
# the free table-side area, outside the current object and bin lanes.  The
# table surface is around world z=0.89 m, so release above both the table and
# the single-bin wall rather than commanding the TCP through the table.
DEFAULT_DROP_POSITION = np.array([-0.55, -0.45, 0.93], dtype=float)
...
FINAL_APPROACH_POSITION_TOLERANCE_M = read_final_approach_position_tolerance_m()
...
VERTICAL_APPROACH_WAYPOINT_MAX_DIST_M = read_vertical_approach_waypoint_max_dist_m()
...
SERVO_MODE_SETTLE_SEC = env_float("GRASP_SERVO_MODE_SETTLE_SEC", "0.10")
# Before handing control from Servo back to MoveIt, wait for genuinely fresh
# joint-state samples to stop changing.  A fixed sleep is not enough here:
# the final Servo command can still be settling when the sleep expires.
RETURN_TO_INITIAL_STABLE_DELTA_RAD = env_float(
    "GRASP_RETURN_TO_INITIAL_STABLE_DELTA_RAD",
    "0.002",
)
RETURN_TO_INITIAL_STABLE_SAMPLES = max(
    2,
    env_int("GRASP_RETURN_TO_INITIAL_STABLE_SAMPLES", "2"),
)
RETURN_TO_INITIAL_STABLE_MIN_DURATION_SEC = env_float(
    "GRASP_RETURN_TO_INITIAL_STABLE_MIN_DURATION_SEC",
    "0.5",
)
RETURN_TO_INITIAL_STABLE_TIMEOUT_SEC = env_float(
    "GRASP_RETURN_TO_INITIAL_STABLE_TIMEOUT_SEC",
    "4.0",
)
RETURN_TO_INITIAL_STABLE_POLL_SEC = max(
    0.01,
    env_float("GRASP_RETURN_TO_INITIAL_STABLE_POLL_SEC", "0.05"),
)
CARTESIAN_START_MAX_ERROR_M = env_float(
    "GRASP_CARTESIAN_START_MAX_ERROR_M",
    "0.03",
)
CARTESIAN_END_MAX_ERROR_M = env_float(
    "GRASP_CARTESIAN_END_MAX_ERROR_M",
    "0.03",
)
CARTESIAN_END_WAIT_SEC = env_float(
    "GRASP_CARTESIAN_END_WAIT_SEC",
    "2.0",
)
DROP_CARTESIAN_AVG_SPEED = env_float(
    "GRASP_DROP_CARTESIAN_AVG_SPEED",
    "0.05",
)
...
VERTICAL_MIN_TCP_ABOVE_TARGET_BOTTOM_M = read_vertical_min_tcp_above_target_bottom_m()
...
SIDE_GRASP_APPROACH_VERTICAL_MARGIN_M = env_float(
    "GRASP_SIDE_APPROACH_VERTICAL_MARGIN_M",
    "0.12",
)
...
GRASP_SCENE_CLEARANCE_TOPIC = (
    os.environ.get("GRASP_SCENE_CLEARANCE_TOPIC", "/scene_description").strip()
    or "/scene_description"
)
APPLE_SCENE_BOUNDS_CENTER_MAX_ERROR_M = _read_positive_finite_float(
    "GRASP_APPLE_SCENE_BOUNDS_CENTER_MAX_ERROR_M",
    "0.020",
)
...
SIDE_GRASP_GEOMETRY_CENTER_BY_OBJECT = {
    "tomato_soup_can": np.array(
        [-0.009169, 0.084018, 0.051006],
        dtype=float,
    ),
    "tuna_fish_can": np.array(
        [-0.026049, -0.022132, 0.013551],
        dtype=float,
    ),
}
...
ROUND_TOP_MAX_APPROACH_ANGLE_DEG = env_float(
    "GRASP_ROUND_TOP_MAX_APPROACH_ANGLE_DEG",
    "20.0",
)
ROUND_TOP_SYMMETRY_YAW_DEG = tuple(range(0, 360, 45))
ROUND_TOP_MAX_CENTER_OFFSET_M = env_float(
    "GRASP_ROUND_TOP_MAX_CENTER_OFFSET_M",
    "0.010",
)
ROUND_TOP_MIN_NORMALIZED_HEIGHT = env_float(
    "GRASP_ROUND_TOP_MIN_NORMALIZED_HEIGHT",
    "0.0",
)
ROUND_TOP_MIN_NORMALIZED_HEIGHT_BY_OBJECT = {
    "apple": env_float(
        "GRASP_APPLE_ROUND_TOP_MIN_NORMALIZED_HEIGHT",
        os.environ.get("GRASP_ROUND_TOP_MIN_NORMALIZED_HEIGHT", "-0.07"),
    ),
}
ROUND_TOP_MAX_NORMALIZED_HEIGHT = env_float(
    "GRASP_ROUND_TOP_MAX_NORMALIZED_HEIGHT",
    "0.20",
)
ROUND_TOP_TARGET_NORMALIZED_HEIGHT = env_float(
    "GRASP_ROUND_TOP_TARGET_NORMALIZED_HEIGHT",
    "0.08",
)
ROUND_TOP_MAX_GRIPPER_OPENING_M = env_float(
    "GRASP_ROUND_TOP_MAX_GRIPPER_OPENING_M",
    "0.08516",
)
ROUND_TOP_TCP_TO_LOWEST_FINGER_Z_M = env_float(
    "GRASP_ROUND_TOP_TCP_TO_LOWEST_FINGER_Z_M",
    "-0.104",
)
ROUND_TOP_TABLE_CLEARANCE_M = env_float(
    "GRASP_ROUND_TOP_TABLE_CLEARANCE_M",
    "0.005",
)
ROUND_TOP_CANDIDATE_COUNT = max(
    1,
    env_int("GRASP_ROUND_TOP_CANDIDATE_COUNT", "8"),
)
...
def get_round_top_min_normalized_height(selected_object_name):
    return ROUND_TOP_MIN_NORMALIZED_HEIGHT_BY_OBJECT.get(
